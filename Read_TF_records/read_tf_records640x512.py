...
def parser(record):
    ...
    image = tf.reshape(image, shape=[640, 512, 3]) # i-f image is already has grayscale
    ...
    tongue_label = tf.reshape(tongue_label, shape=[640, 512, 1])
    print(tongue_label.shape)
    ...
    vein_label = tf.reshape(vein_label, shape=[640, 512, 1])
    print(vein_label.shape)
