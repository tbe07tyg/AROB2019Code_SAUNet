...
def make_tf_records():
    aug_all_input_path = "../data/ALL_data/AugmentedInputs/*"



    # for inputs
    all_input_addrs = glob.glob(aug_all_input_path)  # return a list of strings which are the absolute path of each images in the given folder

    # sort file paths
    all_inputs_addrs = sort_paths(all_input_addrs)



    print("sorted addrs:")
    print("inputs:")
    print(all_inputs_addrs, "\n")
    ...
    # print("--------------------------:",train_c)
    shuffle(all_inputs_addrs)  # shuffle 打乱顺序

    # divide dataset into  train val. test. address
    train_inputs_addrs = all_input_addrs[0: int(0.6 * len(all_input_addrs))]

    val_inputs_addrs = all_input_addrs[int(0.6 * len(all_input_addrs)): int(0.8 * len(all_input_addrs))]

    test_inputs_addrs = all_input_addrs[int(0.8 * len(all_input_addrs)):]


    # unpack shuffled file names
    # (train_inputs_addrs)= zip(* train_c )  # un-zip. 打乱后解包
    print("--------------------------:", all_inputs_addrs)
    write_tf_records_vein_inputs_only("train_inputs_only.tfrecords", train_inputs_addrs)
    write_tf_records_vein_inputs_only("val_inputs_only.tfrecords", val_inputs_addrs)
    write_tf_records_vein_inputs_only("test_inputs_only.tfrecords", test_inputs_addrs)
